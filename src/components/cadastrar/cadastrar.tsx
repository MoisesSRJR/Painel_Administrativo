...
export default function SimpleModal () {
  ...
  const {AddEndereco}=useShop()
    
    const [altdados, setaltdados] = useState ({ 
        status:true, id:nanoid(), nome:"", cnpj:"", senha:"", imagem:"", celular:"", telefone:"", cep:"",
        endereco:"", numero:"", bairro:"", cidade:"", complemento:"", uf:""})
    ...
    function AletraImagem(event: any) {
      const name = event.currentTarget.name
      if(event.target.files) {
        const file = event.target.files[0];
        const reader = new FileReader();
        reader.onloadend = () => {
          setaltdados({...altdados,[name]: String(reader.result)})
        };
        if (file) {
  ...
  const body = (
    <div id="fundo-cadastrar2">
        <div id="divisao1">
            <div>
                <span id="title-cadastrar">Cadastrar estabelecimento</span>
            </div>
            <div>
       ...
       <form onSubmit={OnSubmit}>
        ...
        <div id="divisao3">
            <div>
                ...
                <label id="bt-label"> <img src={uploadimg}/>Carregar imagem <input type="file" name="img"  value={altdados.imagem} onChange={AletraImagem}></input></label>    
            </div>
            <div>
        ...
        <div id="divisao5">
            <div>
                <span id="checkbox">O estabelecimento realiza entrega ?</span>
            </div>
        <FormGroup>
                {<IOSSwitch checked={state.checkedA} onChange={handleChange} name="checkedA"/>}
        </FormGroup>
            <div>
                <span id="checkbox">O estabelecimento permite retirada ?</span>
            </div>
        <FormGroup>
        ...
        <div id="divisao6">
            <button  id="bt1" type="submit">Cadastrar</button>
  ...
  return (
    <div>
      <button id="bt-cadastar" type="button" onClick={handleOpen}>
